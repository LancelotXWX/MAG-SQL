# -*- coding: utf-8 -*-
from main_scripts.utils import *
from main_scripts.chat_manager import ChatManager
from main_scripts.utils import get_gold_columns
...
def contain_date(s):
    r = re.compile('.*\'\d{4}-(0[1-9]|1[0-2])-(0[1-9]|[12]\d|3[01])\'.*')
    if r.match(s) != None:
...
def run_batch(dataset_name, input_file, output_file, db_path, tables_json_path, start_pos=0, log_file=None, dataset_mode='dev', use_gold_schema=False, without_selector=False):
        ...
        for cur_idx, item in tqdm(enumerate(batch)):
            ...
            try:
                ...
                try:
                    ...
                    del user_message['db_content_dict']
                except:
                    pass
    ...
    if dataset_name == "bird":
        ...
        evaluation_file_path = f"{out_dir}/predict_{dataset_mode}_new.json"
        ...
        """
        #evaluation_file_path = f"{out_dir}/mac_pred_codellama7b_epoch5.sql"
        evaluation_file_path = f"{out_dir}/newprompt_mac.sql"
        with open(evaluation_file_path, "w") as f:
            output_json_list = load_jsonl_file(output_file)
            output_json_list = sorted(output_json_list, key=lambda i: i['idx'])
            for output in output_json_list:
                pred_sql = output['pred'].strip()
                pred_sql = replace_multiple_spaces(pred_sql)
...
if __name__ == "__main__":
    ...
    # print each key argument and corresponding value
    for key, value in vars(args).items():
        print(f"{key}: {value}")

    check_all_paths(args)

    # print the args in clean json format
